...
int16_t read_from_eeprom(uint8_t idx)
{
  return (int16_t)(ee_ram[idx * 2 + 1] * 256 + ee_ram[idx * 2 + 0]);
}

void write_to_eeprom(uint8_t idx, int16_t value)
{
  ee_ram[idx * 2 + 1] = ((uint16_t)value) / 256;
  ee_ram[idx * 2 + 0] = ((uint16_t)value) % 256;
  ee_commit();
}